...
Also ships **`privatemode`**, a local Python tool with the same pipeline but no
...
See [Local Python tool](#local-python-tool-privatemode).
...
fence stripping, truncation detection) are built in. The same interface will
later also drive the hosted Privatemode OCR API.

```python
from privatemode import ocr, OCR
...
engine = OCR(base_url="https://api.openai.com/v1", api_key="sk-…", model="gpt-4o")
...
python src/privatemode/cli.py document.pdf -o out.md --pages 0-2 --language de
python src/privatemode/cli.py scan.jpg -f json | jq '.pages[0]'
cd src && python -m privatemode document.pdf              # module form
```
...
src/
  ...
  privatemode/        # Python tool + CLI (local OCR, no server needed)
